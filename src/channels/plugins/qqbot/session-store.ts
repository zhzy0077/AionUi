...
export function loadSession(accountId: string, expectedAppId?: string): SessionState | null {
  ...
  try {
    ...
    if (now - state.savedAt > SESSION_EXPIRE_TIME) {
      console.log(`[qqbot-session-store] Session expired for ${accountId}, age: ${Math.round((now - state.savedAt) / 1000)}s`);
      try {
    ...
    if (expectedAppId && state.appId && state.appId !== expectedAppId) {
      console.log(`[qqbot-session-store] appId mismatch for ${accountId}: saved=${state.appId}, current=${expectedAppId}. Discarding stale session.`);
      try {
        fs.unlinkSync(filePath);
      } catch {
        // 忽略删除错误
    ...
    if (!state.sessionId || state.lastSeq === null || state.lastSeq === undefined) {
      console.log(`[qqbot-session-store] Invalid session data for ${accountId}`);
      return null;
    }

    console.log(`[qqbot-session-store] Loaded session for ${accountId}: sessionId=${state.sessionId}, lastSeq=${state.lastSeq}, appId=${state.appId ?? 'unknown'}, age=${Math.round((now - state.savedAt) / 1000)}s`);
    return state;
  } catch (err) {
    console.error(`[qqbot-session-store] Failed to load session for ${accountId}: ${err}`);
    return null;
  }
}
...
function doSaveSession(state: SessionState): void {
  ...
  try {
    ...
    console.log(`[qqbot-session-store] Saved session for ${state.accountId}: sessionId=${state.sessionId}, lastSeq=${state.lastSeq}`);
  } catch (err) {
    console.error(`[qqbot-session-store] Failed to save session for ${state.accountId}: ${err}`);
  }
}
...
export function clearSession(accountId: string): void {
  ...
  try {
    if (fs.existsSync(filePath)) {
      fs.unlinkSync(filePath);
      console.log(`[qqbot-session-store] Cleared session for ${accountId}`);
    }
  } catch (err) {
    console.error(`[qqbot-session-store] Failed to clear session for ${accountId}: ${err}`);
  }
}
      ...
      if (file.startsWith('session-') && file.endsWith('.json')) {
        ...
        try {
          ...
          if (now - state.savedAt > SESSION_EXPIRE_TIME) {
            fs.unlinkSync(filePath);
            cleaned++;
            console.log(`[qqbot-session-store] Cleaned expired session: ${file}`);
          }
        } catch {
          // 忽略解析错误，但也删除损坏的文件
          try {
